...
CREATE TABLE `class` (
  ...
  KEY `class-major` (`major_key`) USING BTREE,
  KEY `class-department` (`department_key`) USING BTREE,
  CONSTRAINT `class-department` FOREIGN KEY (`department_key`) REFERENCES `department` (`pkid`),
  CONSTRAINT `class-major` FOREIGN KEY (`major_key`) REFERENCES `major` (`pkid`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin ROW_FORMAT=COMPACT COMMENT='InnoDB free: 10240 kB; (`department_key`) REFER `ssm_demo_db/department`(`pkid`)';

-- ----------------------------
-- Records of class
-- ----------------------------
BEGIN;
INSERT INTO `class` VALUES ('085053f0-488c-4bb4-9ac2-20656ae3f50e', '数学1班', '9501ba10-bb3e-4f32-a2fc-4cc39cce3323', '73ab3756-5d9d-4528-809e-14e2834dfe62', NULL);
...
INSERT INTO `class` VALUES ('561a7a6d-b4d3-4505-8a93-ef1995ec9f29', '汉语言1班', '8b1b84e0-cab3-4b8b-a7a9-8c5b453f10f7', '9f00a06b-38e6-4ec4-ad0e-0c4f9d2b7ca5', NULL);
...
CREATE TABLE `course` (
  `pkid` varchar(50) COLLATE utf8mb4_bin NOT NULL COMMENT '主键',
  `course_name` varchar(255) COLLATE utf8mb4_bin NOT NULL COMMENT '课程名称',
  `course_majorkey` varchar(50) COLLATE utf8mb4_bin NOT NULL COMMENT '课程对应的专业',
  `course_teacherkey` varchar(50) COLLATE utf8mb4_bin DEFAULT NULL COMMENT '课程对应的老师',
  PRIMARY KEY (`pkid`)
...
CREATE TABLE `department` (
  ...
  PRIMARY KEY (`pkid`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin ROW_FORMAT=COMPACT COMMENT='InnoDB free: 10240 kB';
...
CREATE TABLE `score` (
  `pkid` varchar(50) COLLATE utf8mb4_bin NOT NULL COMMENT '主键',
  `student_key` varchar(50) COLLATE utf8mb4_bin NOT NULL COMMENT '学生主键',
  `course_key` varchar(255) COLLATE utf8mb4_bin NOT NULL COMMENT '课程主键',
  `exam_score` varchar(50) COLLATE utf8mb4_bin DEFAULT NULL COMMENT '考试成绩'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin;
...
-- ----------------------------
BEGIN;
INSERT INTO `student` VALUES ('2996b841-34c0-48b0-a61e-fe00a6187612', '021446666', '吾浪', '2019', 'a99b523a-fc10-45b1-957e-c3059533d342', '5beae438-4019-4096-8c03-2a41cbbea231', '72c59e68-e1e7-4745-b296-cb173a1788de', '');
...
CREATE TABLE `teacher` (
  `pkid` varchar(50) COLLATE utf8mb4_bin NOT NULL COMMENT '主键',
  `teacher_name` varchar(255) COLLATE utf8mb4_bin NOT NULL COMMENT '教师姓名',
  `sex` varchar(2) COLLATE utf8mb4_bin DEFAULT NULL COMMENT '性别',
  `birthday` varchar(50) COLLATE utf8mb4_bin DEFAULT NULL COMMENT '出生年月',
  `person_id` mediumtext COLLATE utf8mb4_bin COMMENT '身份证号码',
  `phone` varchar(50) COLLATE utf8mb4_bin DEFAULT NULL COMMENT '联系电话',
  `teacher_key` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL COMMENT '教工号',
  PRIMARY KEY (`pkid`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin;
...
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `v_class` AS select `t`.`pkid` AS `pkid`,`t`.`class_name` AS `className`,`t1`.`pkid` AS `departmentKey`,`t1`.`department_name` AS `departmentName`,`t2`.`pkid` AS `majorKey`,`t2`.`major_name` AS `majorName`,`t`.`class_tutor` AS `classtTutor` from ((`class` `t` left join `department` `t1` on((`t`.`department_key` = `t1`.`pkid`))) left join `major` `t2` on((`t`.`major_key` = `t2`.`pkid`)));

-- ----------------------------
-- View structure for v_major
-- ----------------------------
DROP VIEW IF EXISTS `v_major`;
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `v_major` AS select `t`.`pkid` AS `pkid`,`t`.`major_name` AS `majorName`,`t1`.`pkid` AS `departmentKey`,`t1`.`department_name` AS `departmentName` from (`major` `t` left join `department` `t1` on((`t`.`department_key` = `t1`.`pkid`)));

-- ----------------------------
-- View structure for v_student
-- ----------------------------
DROP VIEW IF EXISTS `v_student`;
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `v_student` AS select `t`.`pkid` AS `pkid`,`t`.`xh` AS `xh`,`t`.`xm` AS `xm`,`t`.`year` AS `year`,`t`.`class_key` AS `class_key`,`t`.`major_key` AS `major_key`,`t`.`department_key` AS `department_key`,`t`.`phone` AS `phone`,`t1`.`class_name` AS `class_name`,`t2`.`major_name` AS `major_name`,`t3`.`department_name` AS `department_name` from (((`student` `t` left join `class` `t1` on((`t`.`class_key` = `t1`.`pkid`))) left join `major` `t2` on((`t`.`major_key` = `t2`.`pkid`))) left join `department` `t3` on((`t`.`department_key` = `t3`.`pkid`)));

-- ----------------------------
-- View structure for v_teacher
-- ----------------------------
DROP VIEW IF EXISTS `v_teacher`;
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `v_teacher` AS select `t`.`pkid` AS `pkid`,`t`.`teacher_name` AS `teacherName`,`t`.`teacher_key` AS `teacherKey`,`t`.`sex` AS `sex`,(case when (`t`.`sex` = '1') then '男' when (`t`.`sex` = '2') then '女' end) AS `sexmc`,`t`.`birthday` AS `birthday`,`t`.`person_id` AS `personId`,`t`.`phone` AS `phone` from `teacher` `t`;
